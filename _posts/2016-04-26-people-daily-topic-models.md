...
接下来创建分词器，加载词典， 这里在文件选取上用了<code>file.choose()</code>，从而使得文档调用变成了点窗口的方式，初学者可能对D:\X. ZHANG Master Disk######%%—*Discourse2016\ The Peoples Daily Complied###@$.txt 这样的长路经不习惯。 另外，我暂时没有加载所谓的“用户词典”，仅仅是加载了“停用词典” ，在“停用词典”里，我添加了年份、“人民日报”、“作者”、“版面”等与主题无关的词以及节目名称。
...
```r                                                 comments_segged<- readLines(file.choose(),encoding="UTF-8")
...
```r                                                 del <- term.table < 5| nchar(names(term.table))<2   #把不符合要求的筛出来，使用了一个“或”语句 ( |)
...
```r                                                 top.words10 <- top.topic.words(fit10$topics, 10, by.score=TRUE)
...
```r                                                 theta <- t(apply(fit10$document_sums + alpha, 2, function(x) x/sum(x)))  
